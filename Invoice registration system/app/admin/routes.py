...
def reimburse():
    ...
    if request.method == "POST":
        ...
        with db.cursor() as cur:
            # 带 user_id 的请求是按员工批量核销；只带 invoice_id 的请求继续走
            # 下方单张发票分支。原先只判断 action，导致单张核销分支永远不可达。
            if action == "reimburse" and user_id:
                ...
                cur.execute(
                    "SELECT id, name, total_quota, used_quota FROM users WHERE id=%s FOR UPDATE",
                    (user_id,),
                )
                user = cur.fetchone()
                if not user:
                ...
                # 先锁定该员工所有待核销发票，防止并发请求基于相同余额分配。
                cur.execute(
                    "SELECT id FROM invoices WHERE user_id=%s AND status='Approved' ORDER BY id FOR UPDATE",
                    (user_id,),
                )
                cur.fetchall()
                cur.execute("""
            ...
            cur.execute("SELECT user_id FROM invoices WHERE id=%s", (invoice_id,))
            owner = cur.fetchone()
            if not owner:
                flash("发票不存在。")
                return redirect(url_for("admin.reimburse"))
            # 所有核销路径统一按 users -> invoices 的顺序加锁，降低死锁风险。
            cur.execute("SELECT id FROM users WHERE id=%s FOR UPDATE", (owner["user_id"],))
            cur.fetchone()
            cur.execute("""
                SELECT i.*, u.total_quota, u.used_quota
                FROM invoices i
            ...
            if action == "approve":
                ...
                cur.execute(
                    "SELECT total_quota, used_quota FROM users WHERE id=%s FOR UPDATE",
                    (user_id,),
                )
                quota = cur.fetchone()
                if not quota:
                ...
                remaining_quota = float(quota["total_quota"] or 0) - float(quota["used_quota"] or 0)
                if reimburse_amount > remaining_quota:
                    flash("核销金额不能超过员工当前可核销金额。")
                    return redirect(url_for("admin.reimburse"))
...
def build_consumption_log_query(keyword, invoice_type_id, start_date="", end_date=""):
    sql = """
...
def consumption_records():
    ...
    with db.cursor() as cur:
        ...
        cur.execute("SELECT id, type_name FROM invoice_types WHERE is_active=1 ORDER BY type_name")
        invoice_types = cur.fetchall()
        log_sql, log_params = build_consumption_log_query(keyword, invoice_type_id, start_date, end_date)

        cur.execute(f"SELECT COUNT(*) AS total {log_sql}", log_params)
        total = cur.fetchone()["total"] or 0
        ...
        cur.execute(f"""
            ...
            LIMIT %s OFFSET %s
        """, log_params + [LOG_PAGE_SIZE, offset])
        records = cur.fetchall()

        cur.execute(f"SELECT COALESCE(SUM(rl.amount), 0) AS total_amount {log_sql}", log_params)
        total_amount = cur.fetchone()["total_amount"] or 0

    return render_template(
        "admin/consumption_records.html",
        records=records,
